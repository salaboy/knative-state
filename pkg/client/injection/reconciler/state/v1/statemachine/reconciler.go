...
// controller reconciling v1.StateMachine.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.StateMachine. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.StateMachine) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.StateMachine.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.StateMachine. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.StateMachine) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.StateMachine if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.StateMachine.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.StateMachine) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.StateMachine if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1.StateMachine.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1.StateMachine) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.StateMachine) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.StateMachine resources.
type reconcilerImpl struct {
	...
	Lister statev1.StateMachineLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister statev1.StateMachineLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.StateMachines(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1.StateMachine, desired *v1.StateMachine) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.FlowV1().StateMachines(desired.Namespace)
		...
		updater := r.Client.FlowV1().StateMachines(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.StateMachine) (*v1.StateMachine, error) {

	getter := r.Lister.StateMachines(resource.Namespace)
	...
	patcher := r.Client.FlowV1().StateMachines(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.StateMachine) (*v1.StateMachine, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.StateMachine, reconcileEvent reconciler.Event) (*v1.StateMachine, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
