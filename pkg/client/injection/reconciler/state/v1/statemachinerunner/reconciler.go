...
import (
	...
	versioned "github.com/salaboy/knative-state/pkg/client/clientset/versioned"
	statev1 "github.com/salaboy/knative-state/pkg/client/listers/state/v1"
	zap "go.uber.org/zap"
...
// controller reconciling v1.StateMachineRunner.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.StateMachineRunner. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.StateMachineRunner) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.StateMachineRunner.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.StateMachineRunner. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.StateMachineRunner) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.StateMachineRunner if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.StateMachineRunner.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.StateMachineRunner) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.StateMachineRunner if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1.StateMachineRunner.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1.StateMachineRunner) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.StateMachineRunner) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.StateMachineRunner resources.
type reconcilerImpl struct {
	...
	Lister statev1.StateMachineRunnerLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister statev1.StateMachineRunnerLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.StateMachineRunners(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1.StateMachineRunner, desired *v1.StateMachineRunner) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.FlowV1().StateMachineRunners(desired.Namespace)
		...
		updater := r.Client.FlowV1().StateMachineRunners(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.StateMachineRunner) (*v1.StateMachineRunner, error) {

	getter := r.Lister.StateMachineRunners(resource.Namespace)
	...
	patcher := r.Client.FlowV1().StateMachineRunners(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.StateMachineRunner) (*v1.StateMachineRunner, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.StateMachineRunner, reconcileEvent reconciler.Event) (*v1.StateMachineRunner, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
